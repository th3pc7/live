MainProg.config.Page_modules["admin_movie"] = new PageJS(MainProg, function(Main){
...
function add_match(ev) {
  ev.preventDefault();
  $("#form_add_link input.hidden").val("add_movie");
...
function edit(ev, id, types, defaults){
  ...
  $.post("../action/",{ action: "edit_mv", id: id, type: types, value: value },function(data){
    if(data==="pass"){
      alert("สำเร็จ");
      ref_table();
    }
...
function edit_st(ev, id, value){
  ev.preventDefault();
  $.post("../action/",{ action: "edit_mv_st", id: id, value: value },function(data){
    if(data==="pass"){
      alert("สำเร็จ");
      ref_table();
    }
...
function ref_table(){
  $.post("../action/",{ action: "ref_table_movie" },function(data){
